...
// Initialize session activity if not set
...
$feedback = null;
$selectedSpace = null;
$bookingStep = 'search'; // search, confirm, success
$date = $time = $area = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['confirm_booking'])) {
        ...
        if ($spaceID && $date && $time) {
            ...
            $stmtBook->bind_param("ssssssss", $bookingID, $studentID, $spaceID, $date, $time, $spaceID, $date, $time);
...
<body>
    ...
    <nav class="sidebar">
        ...
        <ul class="menu">
            ...
            <li>
                <a href="Booking.php" class="menutext active">Book Parking</a>
            </li>
            <li>
    ...
    // Simple session warning (optional)
    let sessionWarningShown = false;
    
    setTimeout(() => {
        if (!sessionWarningShown) {
            const extend = confirm("Your session has been active for a while. Would you like to extend it?");
            if (extend) {
                // You could implement an AJAX call to extend session here
